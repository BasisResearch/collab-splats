...
class PoseGraph:
    ...
    def __init__(self, manifold: Literal["sl4", "se3"] = "sl4") -> None:
        ...
        if manifold == "sl4":
            ...
            self._anchor_noise = gtsam.noiseModel.Diagonal.Sigmas(np.full(6, 1e-6, dtype=np.float64))
    ...
    def add_prior(self, node_id: int, H: np.ndarray) -> None:
        ...
        if self._manifold == "sl4":
            ...
            self._graph.add(gtsam.PriorFactorPose3(key, _pose3(H), self._anchor_noise))
    ...
    def add_sequential_edge(self, id_i: int, id_j: int, H_rel: np.ndarray) -> None:
        ...
        if self._manifold == "sl4":
            ...
            self._graph.add(gtsam.BetweenFactorPose3(key_i, key_j, _pose3(H_rel), self._seq_noise))
