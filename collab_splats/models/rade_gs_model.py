...
from collab_splats.utils import get_camera_parameters, depth_double_to_normal
...
class RadegsModel(SplatfactoModel):
    ...
    def get_loss_dict(
        ...
        if (
            ...
            depth_normal_loss = (1 - self.config.depth_ratio) * outputs[
                "depth_normal_error_map"
            ].mean() + self.config.depth_ratio * outputs[
                "middepth_normal_error_map"
            ].mean()
    ...
    def _prefilter_voxel(self, camera_params: Dict[str, torch.Tensor]):
        ...
        assert camera_params["viewmats"].shape == (C, 4, 4), camera_params[
            "viewmats"
        ].shape
