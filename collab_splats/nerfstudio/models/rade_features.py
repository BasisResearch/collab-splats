...
from typing import Type, Dict, Union, List, Optional, Tuple
...
class RadegsFeaturesModel(RadegsModel):
    ...
    def populate_text_encoder(self):
        ...
        if feature_type in _QUERYABLE_FEATURE_TYPES:
            ...
            self.add_module("text_encoder", self.text_encoder)  # handles device placement
    ...
    def get_outputs_for_camera(
        ...
        if self.similarity_fx is not None:
            # Decoder outputs are not unit-norm — renormalize per patch (dim=0 over C in CHW)
            # so the downstream einsum is true cosine similarity.
            feats = F.normalize(decoded_features_dict[self.main_features_name], dim=0)
            similarity_map = self.similarity_fx(
                features=feats,
                positive=self.positive_queries,
                negative=self.negative_queries,
                reduction=self.config.similarity_method,
            )
            ...
            assert similarity_map.ndim == 2  # (H, W) now
            if similarity_map.shape[:2] != outs["rgb"].shape[:2]:
                similarity_map = (
                    F.interpolate(
                        similarity_map[None, None],  # (H,W) -> (1,1,H,W)
