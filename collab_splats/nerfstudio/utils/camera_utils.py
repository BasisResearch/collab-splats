...
class ColmapCamera:
    def __init__(
        ...
        self.world_view_transform = torch.tensor(get_world2view_transform(R, T, trans, scale)).transpose(0, 1).cuda()
        self.projection_matrix = (
            get_projection_matrix(znear=self.znear, zfar=self.zfar, fovx=self.fovx, fovy=self.fovy)
            .transpose(0, 1)
            .cuda()
        )
...
def build_rotation(quats: torch.Tensor) -> torch.Tensor:
    ...
    """
    norm = torch.sqrt(
        quats[:, 0] * quats[:, 0] + quats[:, 1] * quats[:, 1] + quats[:, 2] * quats[:, 2] + quats[:, 3] * quats[:, 3]
    )
    q = quats / norm[:, None]
