...
class MaskCLIPExtractor(BaseFeatureExtractor):
    ...
    def compute_similarity(
        ...
        if method == "standard":
            ...
            pos_similarities = raw_similarities[
                :num_positive
            ]  # (num_positive, num_pixels)
            neg_similarities = raw_similarities[
                num_positive:
            ]  # (num_negative, num_pixels)
            ...
            broadcasted_pos = avg_pos_similarity.expand(
                neg_similarities.shape[0], -1
            )  # (num_negative, num_pixels)

            # Create pairs: positive vs each negative
            paired_similarities = torch.cat(
                [broadcasted_pos, neg_similarities], dim=0
            )  # (2*num_negative, num_pixels)
            ...
            pos_pair_probs = probs[
                : neg_similarities.shape[0]
            ]  # First half are positive probs
            pos_similarity = pos_pair_probs.min(dim=0)[
                0
            ]  # Take minimum across all pairs
...
class DINOFeatureExtractor(BaseFeatureExtractor):
    def __init__(
        ...
        self.model = load_torchhub_model("facebookresearch/dinov2", model_name).to(
            device
        )
        self.model.eval()
    ...
    def preprocess(self, image) -> torch.Tensor:
        ...
        image, target_H, target_W = interpolate_to_patch_size(
            image, self.model.patch_size
        )
    ...
    def reshape(
        self, features: torch.Tensor, target_H: int, target_W: int
    ) -> torch.Tensor:
