...
**Goal:** Add `scripts/reconstruct.py` CLI + `configs/reconstruction/` hierarchy so any dataset can be run end-to-end (pointcloud → semantics) with a single command, and run all 10 fieldwork datasets with `vggt_omega`.
...
| Create | `scripts/reconstruct.py` | CLI entry point |
...
### Task 5: Write failing tests for `scripts/reconstruct.py`
...
### Task 6: Implement `scripts/reconstruct.py`
...
```bash
chmod +x scripts/reconstruct.py
```
...
If any test fails, fix `scripts/reconstruct.py` (not the tests) and re-run.
...
```bash
/opt/conda/envs/reconstruction/bin/python scripts/reconstruct.py --help
```
...
SCRIPT=/workspace/collab-splats/scripts/reconstruct.py
