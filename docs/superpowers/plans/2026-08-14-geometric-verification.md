...
- **Outdoor benchmark: Oxford Spires** (`https://dynamic.robots.ox.ac.uk/datasets/oxford-spires/`) — **deferred to the follow-on validation step**; chosen 2026-08-16 over Newer College (grayscale/IR cameras) and Tanks & Temples (orbit motion, no GT poses). Matches our production regime: backpack walking capture, color global-shutter 1440×1080 @ 126° FoV, 24 sequences over Oxford landmarks. Dual GT: Leica RTC360 TLS map (cloud/mesh accuracy) + cm trajectory GT on 12 core sequences (Tier 1 pose validation) + published NeRF/3DGS baselines for downstream splat comparison. Caveats: CC BY-NC-SA (NonCommercial — conscious call needed before productized use), fisheye needs undistortion to PINHOLE, GT range caps ~60 m. It enters at validation of a chosen fix — the immediate diagnosis runs reference-free on our own outdoor scenes.
