...
## 2. Slow Frame Extraction (FPS mode)
...
**Problem:** FPS mode produces frame counts proportional to video length (short clip vs 10-min video = wildly different counts at the same FPS). No video-length-agnostic cap exists.
...
- Add `max_frames: int | None = None` param to `sample_frames_fps` (stop appending when `max_frames is not None and len(frames) >= max_frames`). `None` = no cap; existing callers unaffected.
...
## 5. Image Displayed Upside Down

**Problem:** Orientation fix was specced in `2026-04-20-dashboard-fixes-design.md` but never committed — no dedicated commit exists in git log. Both sampling functions open `cv2.VideoCapture` without setting orientation auto-correction, so container rotation metadata is ignored.

**Fix:** Add `cap.set(cv2.CAP_PROP_ORIENTATION_AUTO, 1)` immediately after `cv2.VideoCapture(video_path)` in both `sample_frames_fps` and `sample_frames_optical_flow`. This instructs OpenCV to apply the rotation encoded in the container metadata.
...
| File | Changes |
|------|---------|
| `collab_splats/utils/frame_sampling.py` | Sequential read in `sample_frames_fps`; `max_frames` param; `CAP_PROP_ORIENTATION_AUTO=1` in both functions |
| `collab_splats/dashboard/semantics.py` | `frame_slider` init `end=1`; progress bar moved into `frames_controls`; `max_frames_slider` widget + wiring |
