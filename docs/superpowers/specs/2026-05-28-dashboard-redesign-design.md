...
**Purpose:** Configure and run the reconstruction pipeline (`scripts/reconstruct.py`) for the current session's video/frames.
...
### Execution
Spawns `scripts/reconstruct.py` as subprocess with config written to a temp YAML. Streams stdout/stderr to log panel (right column) and global progress strip. Kill button appears while running.
