...
- **`trajectory_io.py` → `collab_splats/geometry/trajectory_io.py`.** Pure TUM/KITTI trajectory I/O (`read_tum`, `write_tum`, `kitti_file_to_w2c`, `kitti_3x4_flat_to_w2c`, `w2c_to_kitti_3x4_flat`, `invert_poses`-style helpers) with zero eval-specific content — a natural sibling of `geometry/transforms.py`. Currently has no consumer outside `evals/` (moving it is an architecture placement, not a response to existing cross-package reuse), which is exactly what the user asked for: library-shaped code belongs in the library. Add its public functions to `geometry/__init__.py`'s imports/`__all__`, matching how `transforms.py`'s functions are already exposed there.
