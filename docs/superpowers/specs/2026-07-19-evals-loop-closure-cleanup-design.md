...
Three `evals/` root modules turned out to be library code, not eval-harness code — identified by asking "does this module contain any eval-specific concept (ground truth, benchmark comparison), or is it pure geometry/diagnostics that a production caller could use?":

- **`trajectory_io.py` → `collab_splats/geometry/trajectory_io.py`.** TUM/KITTI trajectory I/O (`read_tum`, `write_tum`, `kitti_file_to_w2c`, `kitti_3x4_flat_to_w2c`, `w2c_to_kitti_3x4_flat`) with zero eval-specific content — a natural sibling of `geometry/transforms.py`, and no equivalent text-format I/O exists there today. Currently has no consumer outside `evals/` (moving it is an architecture placement, not a response to existing cross-package reuse), which is exactly what the user asked for: library-shaped code belongs in the library. Add its public functions to `geometry/__init__.py`'s imports/`__all__`, matching how `transforms.py`'s functions are already exposed there.
  - **Bonus dedup: delete `trajectory_io._invert_se3`.** It hand-rolls the same batched SE(3) inverse (`R^T`, `-R^T@t`) as `transforms.invert_poses`, just restricted to the `(N,4,4)` case — `invert_poses` already handles arbitrary leading batch dims, so it's a strict superset. All four call sites in `trajectory_io.py` (`write_tum`, `read_tum`, `kitti_3x4_flat_to_w2c`, `w2c_to_kitti_3x4_flat`) switch to `invert_poses`; `_check_poses` stays (no equivalent shape-validation helper exists in `transforms.py`).
  - Side effect: kills the `importlib.util.spec_from_file_location` workaround in `evals/runners/build_parity_table.py` (there to dodge `trajectory_io` not being a real import path from `runners/`) — becomes a normal `from collab_splats.geometry.trajectory_io import ...`.
- **`reconstruction_quality.py` → `collab_splats/geometry/loop_closure/diagnostics.py`.** Its aggregator, `compute_alignment_metrics(lc_creator)`, reads `_lc_submaps` / `_lc_all_matches` / `_lc_precorrection_extrinsics` / `_lc_corrected_extrinsics` directly off a post-run `LoopClosure`'s `base` — these are private attributes set inside `wrapper.py` itself (lines 381-398). This isn't an eval-comparison function operating on file pairs like `metrics.py`; it's a diagnostics extension of `geometry/loop_closure` that happens to have been placed in `evals/`. Its three public metrics (`loop_match_residual`, `submap_boundary_gap`, `pointcloud_chamfer`) and internal helpers (`_cam_positions`, `_symmetric_chamfer`, `_global_frame`) move as-is. `eval_gt.py`'s inline import (line 405, already inline because it's optional/heavy-path) updates to `from collab_splats.geometry.loop_closure.diagnostics import compute_alignment_metrics`.
...
- **Test relocation** (mirrors `tests/` ↔ `collab_splats/` convention): `tests/evals/test_trajectory_io.py` → `tests/geometry/test_trajectory_io.py`; `tests/evals/test_reconstruction_quality.py` → `tests/geometry/loop_closure/test_diagnostics.py`; `tests/evals/test_ate_utils_tum.py`'s cases merge into the existing `tests/evals/test_datasets.py` (testing the relocated GT loader), then the file is deleted. `test_metrics.py`/`test_eval_compare.py` stay in `tests/evals/`, just update their `trajectory_io` import path.
