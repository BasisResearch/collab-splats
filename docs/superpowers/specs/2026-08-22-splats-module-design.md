...
| Primitive | `primitive: 2dgs \| 3dgs`, default `2dgs` | 2DGS gives normals + normal/distortion losses → geometry. 3DGS has fast kernel. One trainer, branch only at the rasterizer call. |
| Losses | photometric (always on, `(1-0.2)·L1 + 0.2·SSIM`), optional `depth`, `normal_consistency`, `distortion`, each `{weight, start}` | All upstream-proven. `normal_consistency`/`distortion` are 2DGS-only → `ValueError` under 3dgs. No `end`; no ramps. |
...
| Off | `app_opt` (view-dependent exposure breaks RGB/depth handoff), MCMC, bilateral grid, compression, distributed, holdout split, LPIPS | Not needed for train-view → mesh use case. Ablation tooling belongs in a later `evals/scripts/eval_splats.py`. |
...
- `SplatsConfig` dataclass: `primitive`, `max_steps`, `pose_opt`, `losses: dict[str, LossSpec]`.
  ...
  under 3dgs.
- `_load_scene(scene_dir)`: `PointcloudResult` → c2w `(N,4,4)`, K `(N,3,3)` (ordered by
  `image_paths`); `FrameStore` → RGB uint8 `(N,H,W,3)`; `points3D` xyz+rgb for init; per-frame
  sparse `(pixel_xy, depth)` targets from point2D↔point3D tracks. Upstream world-space
  normalization (similarity `T_norm`, stored, inverted at export). All tensors resident on GPU
  (scenes ≤ ~300 frames).
...
- `_render(primitive, splats, c2w, K, H, W, sh_degree)`: 3dgs →
  `rasterization(render_mode="RGB+ED", antialiased=True, absgrad=True)`; 2dgs →
  `rasterization_2dgs(...)`. Returns `(rgb, alpha, depth, normal|None, surf_normal|None,
  distort|None, info)`. Only function touching rasterizers.
...
def compute_losses(step, out, batch, specs, scene_scale) -> tuple[Tensor, dict[str, float]]
```
Photometric always. Optional loss active iff `name in specs and weight > 0 and step >= start`.
`depth`: `depth_l1_loss` on sparse targets (disparity space, `scene_scale`). `normal_consistency`:
`normal_cosine_loss(normal, surf_normal * alpha.detach())`. `distortion`: `distort.mean()`.
Returns total + per-loss floats for logging/report.
...
splats:
  enabled: false
  primitive: 2dgs            # 2dgs | 3dgs
  max_steps: 30000
  pose_opt: false
  losses:                    # photometric always on; entries are optional, {weight, start}
    depth: {weight: 0.01}
...
splats.zarr
  ...
  normal  (N,H,W,3) float32  2dgs only — ABSENT under 3dgs, never zeros
  alpha   (N,H,W)   float32
...
- `primitive: 3dgs` with `normal_consistency` or `distortion` weight > 0 → `ValueError`.
...
- `tests/splats/test_trainer.py` — CUDA: synthetic 8-frame 64×64 scene with 200 points,
  `max_steps=50`, both primitives. Asserts all outputs exist, zarr schema (normal absent for
  3dgs), depth back in COLMAP scale, `pose_opt: true` changes `c2w`, `pose_opt: false`
...
- Tutorial scene (`data/tutorial/`): 2dgs vs 3dgs step time and `splats_quality_report` PSNR.
