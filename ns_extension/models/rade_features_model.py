...
class RadegsFeaturesModelConfig(RadegsModelConfig):
    ...
    feature_loss_lambda: float = 1.0

    resize_factor: float = 8.0
    """Factor to resize the features to the model dimensionality."""
...
class RadegsFeaturesModel(RadegsModel):
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        # Hack to get features into model gsplat for rendering
        fused_features = torch.cat((colors_crop, distill_features_crop), dim=1)
        ...
        features = render[:, ..., 3:3 + self.config.feature_latent_dim]
