...
This module provides preprocessing tools for intelligent frame selection from videos
...
2. Motion-based selection: Uses Lucas-Kanade optical flow to detect camera motion
3. Coverage-based selection: Ensures spatial and visual diversity
4. Adaptive thresholding: Auto-adjusts parameters based on video statistics

Usage:
    from preproc_utils import OpticalFlowFrameSelector
    ...
    # Process video
    selected_indices, metrics = selector.process_video('path/to/video.mp4')

    # Or process from directory
    selected_indices, metrics = selector.process_image_directory('path/to/images/')
