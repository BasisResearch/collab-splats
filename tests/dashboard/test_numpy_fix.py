...
def test_dashboard_import_no_numpy_warning():
    ...
    result = subprocess.run(
        [
            ...
            "error::UserWarning",
            "-W",
            "ignore:Overwriting:UserWarning",
            "-c",
            "from collab_splats.dashboard import SplatsApp",
        ],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0, f"Dashboard import raised UserWarning (likely NumPy ABI):\n{result.stderr}"
