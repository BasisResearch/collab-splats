"""The lifted store must never be mistaken for the extractor's 2D patch cache.

semantics_dir is flat: it holds BOTH {extractor}.zarr (2D patch cache, (N, D, H_p, W_p)) and
{extractor}_lifted.zarr (lifted per-point codes, (P, latent)). Both are named after the same
extractor, so the `_lifted` suffix is the only discriminator — and Path.glob yields in
os.scandir order, so an unfiltered wildcard picks either one nondeterministically.
"""
...
def _write_both_stores(tmp_path):
    ...
    lifted = zarr.open(str(tmp_path / LIFTED), mode="w")
    lifted["features"] = np.zeros((7, 4), dtype=np.float32)


def _force_lifted_first(monkeypatch):
    """Pin the adversarial glob order: the lifted store yielded first.
    ...
    def ordered_glob(self, pattern, *args, **kwargs):
        return iter(sorted(real_glob(self, pattern, *args, **kwargs), key=lambda p: p.name != LIFTED))
...
def test_load_feature_maps_ignores_lifted_features_store(tmp_path, monkeypatch):
    ...
    _force_lifted_first(monkeypatch)
...
def test_cache_extractor_name_comes_from_the_2d_cache_not_the_lifted_store(tmp_path, monkeypatch):
    """The write paths recover the extractor from the cache stem — never from `*_lifted`."""
    _write_both_stores(tmp_path)
    _force_lifted_first(monkeypatch)

    assert pipeline.cache_extractor_name(tmp_path) == "talk2dino"
