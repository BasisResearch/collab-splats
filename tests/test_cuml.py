...
def test_gpu():
    ...
    try:
        ...
        for i in range(n_devices):
            ...
            print(
                f"        Memory: {free_mem / 1e9:.1f}GB free / {total_mem / 1e9:.1f}GB total"
            )
    ...
    try:
        ...
        if np.array_equal(result, expected):
            ...
            print("      ✗ CuPy operation gave wrong result")
            return False
    ...
    try:
        ...
        if len(unique_labels) == n_clusters:
            print("      ✓ GPU KMeans completed successfully")
            print(f"        - Clustered {n_samples:,} points in {gpu_time:.3f}s")
            print(
                f"        - Found {len(unique_labels)} clusters (expected {n_clusters})"
            )
            print(f"        - Performance: {n_samples / gpu_time:.0f} points/sec")
        else:
            print(
                f"      ⚠ KMeans returned {len(unique_labels)} clusters (expected {n_clusters})"
            )
            return False
        ...
        _ = kmeans_cpu.fit_predict(X_cpu)
        ...
        if speedup > 1.5:
            print("      ✓ GPU is significantly faster than CPU")
        elif speedup > 0.5:
            print("      ⚠ GPU speedup is marginal (may be overhead for small data)")
        else:
            print("      ⚠ GPU is slower than CPU (check GPU utilization)")
