...
| `Video-Depth-Anything/` | `DepthAnything/Video-Depth-Anything` @ `4f5ae23` (source only — no weights here) | `setup.sh` (InstantSfM block) | Metric video depth for the InstantSfM backend (`collab_splats/pointcloud/vda.py::generate_vda_depth`). `sys.path` insert of the clone root (it imports a top-level `utils` namespace package). The vitl checkpoint is fetched from HF `depth-anything/Metric-Video-Depth-Anything-Large` on first use and cached under `HF_HOME`. Code Apache-2.0; **vitl weights CC-BY-NC-4.0** (research use, same as InstantSfM). |
