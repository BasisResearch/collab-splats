...
| `Video-Depth-Anything/` | `DepthAnything/Video-Depth-Anything` @ `4f5ae23` + `checkpoints/metric_video_depth_anything_vitl.pth` (HF `depth-anything/Metric-Video-Depth-Anything-Large`) | `setup.sh` (InstantSfM block) | Metric video depth for the InstantSfM backend (`collab_splats/pointcloud/sfm.py::generate_vda_depth`). `sys.path` insert of the clone root (it imports a top-level `utils` namespace package). Code Apache-2.0; **vitl weights CC-BY-NC-4.0** (research use, same as InstantSfM). |
